...
def initPlanSettings():
    ...
    if isGitHubBuild(): settings = github.initPlanSettings(settings)
...
def initCompilerSettings(planSettings):
    ...
    if isGitHubBuild(): settings = github.initCompilerSettings(planSettings)
    return settings
...
def isGitHubBuild():
    if 'GITHUB_WORKFLOW' in os.environ.keys():
        return True
    else:
        return False
